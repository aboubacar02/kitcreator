...
import { Loader2, Check, Heading, CircleAlert, RotateCcw } from 'lucide-react'
...
export default function TitleAnalyzer() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-semibold tracking-tight text-zinc-950">{t('tools.title')}</h2>
      ...
      <div ref={stateRef} className="scroll-mt-24 space-y-4">
        ...
        {!loading && !error && result && (
          <div className="space-y-3">
            ...
            <ResultCard title={t('result.analyze')} content={result} />
          </div>
        )}
      </div>
    </div>
